...
//マクロ定義
//=============================================================================
#define STR_HIGHT 18	//文字の高さ
#define STR_WIDTH 0		//文字の幅
#define FONT_WEIGHT 0	//フォントの太さ
...
{
	char aMoji[STR_LENGTH];

	ZeroMemory(aMoji, sizeof(aMoji));
	...
	vsprintf(aMoji, pFmt, list);

	//文字列を結合する
	strcat(m_aString[nLayer], aMoji);
...
void CDebugFont::Draw(void)
{
	...
	if (m_pFont)
	{
		for (int nCntLayer = 0; nCntLayer < DEBUG_LAYER_MAX; nCntLayer++)
		{
			if (nCntLayer == 0)
			{
				SetRect(&rect, 0, STR_HIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
