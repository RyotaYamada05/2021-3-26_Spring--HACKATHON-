...
{
	//タイトル背景用ポリゴンの生成
	m_apPolygon[TITLE_TYPE_BG] = CPolygon::Create(SCREEN_CENTER_POS, SCREEN_SIZE, CPolygon::TEX_TYPE_TITLE);

	//タイトルロゴ用ポリゴンの生成
	m_apPolygon[TITLE_TYPE_LOGO] = CPolygon::Create(D3DXVECTOR3(SCREEN_WIDTH/2,-50.0f,0.0f), 
		D3DXVECTOR3(900.0f,600.0f,0.0f), CPolygon::TEX_TYPE_TITLE_LOGO);
	...
	//単位ベクトルを求める
	D3DXVECTOR3 VectorNor = Vector / fVectorSize; //D3DXVECTOR3(Vector.x / fVectorSize, Vector.y / fVectorSize, 0.0f);

	m_move = VectorNor * LOGO_MOVE_SPEED; //D3DXVECTOR3(VectorNor.x * LOGO_MOVE_SPEED, VectorNor.y * LOGO_MOVE_SPEED, 0.0f);
